        ...
        public async Task<int> UpdateFromAucSupplierOtherChargesAsync()
        {
            ...
            try
            {
                ...
                using (var cmd = new SqlCommand(@"
                    ...
                    WHERE PB_SupplerChargesId IS NOT NULL
                    ORDER BY UPDATEID", sqlConnection))
                {
                    ...
                    while (await reader.ReadAsync())
                    {
                        sourceData.Add(new AucSupplierOtherChargesRow
                        {
                            ...
                            GSTPer = reader.IsDBNull(7) ? null : reader.GetDecimal(7),
                            GSTAmount = reader.IsDBNull(8) ? null : reader.GetDecimal(8),
                            UPDATEID = reader.IsDBNull(9) ? 0 : reader.GetInt32(9)
                        });
                    }
                }
                ...
                foreach (var record in sourceData)
                {
                    try
                    {
                        ...
                        // UPSERT based on unique key: (event_id, user_price_bid_lot_charges_id, supplier_id)
                        using var upsertCmd = new NpgsqlCommand(@"
                            ...
                            ON CONFLICT (event_id, user_price_bid_lot_charges_id, supplier_id) DO UPDATE SET
                                ...
                                supplier_lot_charges_file_name = EXCLUDED.supplier_lot_charges_file_name,
                                modified_date = CURRENT_TIMESTAMP", pgConnection);

                        upsertCmd.Parameters.AddWithValue("@event_id", (object?)record.EVENT_ID ?? DBNull.Value);
                        upsertCmd.Parameters.AddWithValue("@user_price_bid_lot_charges_id", (object?)record.PB_BuyerChargesId ?? DBNull.Value);
                        upsertCmd.Parameters.AddWithValue("@price_bid_charges_id", priceBidChargesId.Value);
                        upsertCmd.Parameters.AddWithValue("@supplier_id", (object?)record.SUPPLIER_ID ?? DBNull.Value);
                        upsertCmd.Parameters.AddWithValue("@percentage", (object?)record.Percentage ?? DBNull.Value);
                        upsertCmd.Parameters.AddWithValue("@basic_lot_charges_amount", (object?)record.Amount ?? DBNull.Value);
                        upsertCmd.Parameters.AddWithValue("@tax_master_id", (object?)taxMasterId ?? DBNull.Value);
                        upsertCmd.Parameters.AddWithValue("@tax_percentage", (object?)record.GSTPer ?? DBNull.Value);
                        upsertCmd.Parameters.AddWithValue("@tax_amount", (object?)record.GSTAmount ?? DBNull.Value);
                        upsertCmd.Parameters.AddWithValue("@total_lot_charges_amount", (object?)totalLotChargesAmount ?? DBNull.Value);
                        upsertCmd.Parameters.AddWithValue("@supplier_lot_charges_file_name", (object?)record.LotChargeFileName ?? DBNull.Value);
