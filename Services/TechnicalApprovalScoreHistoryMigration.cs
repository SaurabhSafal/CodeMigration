...
public class TechnicalApprovalScoreHistoryMigration : MigrationService
{
    private const int BATCH_SIZE = 1000;
    private readonly ILogger<TechnicalApprovalScoreHistoryMigration> _logger;
...
SELECT
    ...
    AddedBy,
    AddedDate,
    IsDeleteforExcelReport,
    IsReopen
FROM TBL_TechApprovalScoreHistory
";

    protected override string InsertQuery => @"
INSERT INTO technical_approval_score_history (
...
ON CONFLICT (technical_approval_score_id) DO UPDATE SET
    ...
    public TechnicalApprovalScoreHistoryMigration(IConfiguration configuration, ILogger<TechnicalApprovalScoreHistoryMigration> logger) : base(configuration)
    {
        _logger = logger;
    }
    ...
    public override List<object> GetMappings()
    {
        ...
        {
            new { source = "TechApprovalScoreId", logic = "TechApprovalScoreId -> technical_approval_score_id (Direct)", target = "technical_approval_score_id" },
    ...
    protected override async Task<int> ExecuteMigrationAsync(SqlConnection sqlConn, NpgsqlConnection pgConn, NpgsqlTransaction? transaction = null)
    {
        ...
        {
            var techApprovalScoreId = reader["TechApprovalScoreId"] ?? DBNull.Value;
            ...
            var addedBy = reader["AddedBy"] ?? DBNull.Value;
            var addedDate = reader["AddedDate"] ?? DBNull.Value;
            var isDeleteforExcelReport = reader["IsDeleteforExcelReport"] ?? DBNull.Value;
            var isReopen = reader["IsReopen"] ?? DBNull.Value;
            ...
            {
                ["technical_approval_score_id"] = techApprovalScoreId,
                ["event_id"] = eventId,
                ["event_item_id"] = pbId,
    ...
    private async Task<int> InsertBatchAsync(NpgsqlConnection pgConn, List<Dictionary<string, object>> batch, NpgsqlTransaction? transaction, int batchNumber)
    {
        ...
        var sql = $@"INSERT INTO technical_approval_score_history ({string.Join(", ", columns)}) 
